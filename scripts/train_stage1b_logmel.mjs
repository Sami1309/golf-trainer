...
const MODEL_PATH = join(ROOT, 'frontend', 'models', 'stage1b_logmel.json');
const DEPLOYED_MODEL_PATH = join(ROOT, 'frontend', 'models', 'stage1b_detector.json');
const REPORT_PATH = join(ROOT, 'data', 'stage1b_logmel_report.json');
const DEPLOYED_REPORT_PATH = join(ROOT, 'data', 'stage1b_detector_report.json');
...
function standardize(row, scaler) {
  const out = new Array(row.x.length);
  for (let i = 0; i < row.x.length; i++) out[i] = (row.x[i] - scaler.mean[i]) / scaler.std[i];
  return out;
}
...
function trainLogistic(rows, indices, scaler, opts = {}) {
  ...
  let bias = 0;
  const epochs = opts.epochs ?? 1600;
  const baseLr = opts.lr ?? 0.045;
  const l2 = opts.l2 ?? 0.08;
  const xs = indices.map(idx => standardize(rows[idx], scaler));
  const ys = indices.map(idx => rows[idx].y);
  ...
  const sampleWeights = ys.map(y => y ? posWeight : negWeight);
  const norm = sampleWeights.reduce((acc, v) => acc + v, 0);

  for (let epoch = 0; epoch < epochs; epoch++) {
    for (let j = 0; j < d; j++) gradW[j] = l2 * weights[j];
    let gradB = 0;
...
function predict(row, model, scaler) {
  ...
  for (let j = 0; j < row.x.length; j++) {
    z += model.weights[j] * ((row.x[j] - scaler.mean[j]) / scaler.std[j]);
  }
  return sigmoid(z);
}

function groupKey(row) {
  return row.groupId ?? row.sourceId ?? row.sourcePath ?? row.localPath;
}
...
function chooseThresholdByWorstFold(foldScored, minRecall = 0.95) {
  ...
  if (safe.length) return safe.reduce((best, cur) => cur.threshold > best.threshold ? cur : best);
...
async function loadRows() {
  ...
  for (let i = 0; i < manifestRows.length; i++) {
    ...
    if ((i + 1) % 100 === 0) console.log(`Extracted log-mel features ${i + 1}/${manifestRows.length}`);
...
async function loadBaselineReport() {
  const candidates = [
    join(ROOT, 'data', 'stage1b_handcrafted_report.json'),
    join(ROOT, 'data', 'stage1b_detector_report.json'),
  ];
  try {
    return JSON.parse(await readFile(candidates[0], 'utf8'));
  } catch {
    try {
      return JSON.parse(await readFile(candidates[1], 'utf8'));
    } catch {
      return null;
    }
  }
...
if (!rows.length) throw new Error(`No prepared examples found at ${PREPARED_MANIFEST_PATH}`);
...
const report = {
  ...
  deployedModelPath: DEPLOYED_MODEL_PATH.replace(ROOT + '/', ''),
  ...
  trainScoreSeparation,
  trainOnlyMetrics,
  baselineHandcraftedCv: baselineReport?.cv ?? null,
  ...
  hardestFalsePositivesAtThreshold: oof
    ...
    .map(s => ({
      ...
      localPath: rows[s.idx].localPath,
      category: rows[s.idx].category,
      centerSec: rows[s.idx].centerSec,
    })),
};
...
await writeFile(DEPLOYED_MODEL_PATH, JSON.stringify(modelOut, null, 2));
await writeFile(REPORT_PATH, JSON.stringify(report, null, 2));
await writeFile(DEPLOYED_REPORT_PATH, JSON.stringify(report, null, 2));

const baseline = baselineReport?.cv;
console.log(`Stage 1b log-mel trained: ${nPos} positives, ${nNeg} negatives, ${LOG_MEL_FEATURE_NAMES.length} features`);
...
if (baseline) {
  console.log(`Handcrafted baseline CV  P=${baseline.precision.toFixed(3)} R=${baseline.recall.toFixed(3)} F1=${baseline.f1.toFixed(3)} specificity=${baseline.specificity.toFixed(3)} worstFoldSpec=${baseline.worstSpecificity.toFixed(3)}`);
}
...
console.log(`Wrote ${DEPLOYED_MODEL_PATH}`);
console.log(`Wrote ${REPORT_PATH}`);
console.log(`Wrote ${DEPLOYED_REPORT_PATH}`);
